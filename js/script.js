...
//Función que calcula los lm/l a partir de los litros y vatios dados
...
//Función para ocultar la temperatura
...
//Función para generar de forma aleatoria las imágenes de la sección conócenos
function infoSection(){
  document.getElementById("section").innerHTML = '';
  let setPlants = new Set();
  let setFishes = new Set();

  var plantsNumber = Math.floor(Math.random() * 2 + 2);
  var fishesNumber = 5-plantsNumber;

  do{
    var idFish = Math.floor(Math.random() * fishes.records.length);
    setFishes.add(idFish);
  }while (setFishes.size < fishesNumber)

  do{
    var idPlant = Math.floor(Math.random() * plants.records.length);
    setPlants.add(idPlant);
  }while (setPlants.size < plantsNumber)

  setFishes.forEach(element => document.getElementById("section").innerHTML += "<img src='"+fishes.records[element].imagen+"' data-toggle='modal' data-target='#modal' onclick ='loadFishesModelInfo("+fishes.records[element].id+")'>");
  setPlants.forEach(element => document.getElementById("section").innerHTML += "<img src='"+plants.records[element].imagen+"' data-toggle='modal' data-target='#modal' onclick ='loadPlantsModelInfo("+plants.records[element].id+")'>");
...
//Función para mostrar todas las plantas
...
//Función para mostrar todos los peces
...
//Funcion para obtener los datos insertados por el usario
...
//Función para comprobar si los campos están vacíos o no
...
//Función para cargar las plantas a partir de la información dada
...
//Función para cargar los peces a partir de la información dada
...
//Función para cargar la información de cada planta
...
//Función para cargar la información de cada pez
function loadFishesModelInfo(id){
  ...
  for(var i = 0; i < fishes.records.length; i++){
   ...
   document.getElementById("modal-content").display ="block";
}

//Función para crear consejos
function advices(){
...
//Función para obtener los valores comunes
